...
- `PAOS-TEST-0008` Adversarial review is mandatory for all policy writing, policy expansion, and policy rewriting — not only for new rule proposals. The review must explicitly and separately address three named categories: (1) **Gaps** — what the rule fails to cover, who it fails to protect, which scenarios fall outside its scope, and which harms go unremedied; (2) **Edge cases** — boundary conditions, unusual fact patterns, and populations the rule treats inconsistently or leaves in an indeterminate state; and (3) **Loopholes** — paths by which a regulated actor can satisfy the letter of the rule while defeating its purpose, including definitional workarounds, jurisdictional escape routes, and structural evasion. These three categories must be reviewed and documented separately, not collapsed into a general adversarial review note. In addition to the named triad, the review must also address: unintended consequences; foreseeable abuse paths; perverse incentives; and burden-shifting. This requirement applies to: additions of new rules to an existing pillar or family; rewrites or revisions of existing rules; expansions of scope, coverage, or enforcement; and any change that substantively alters a right, duty, prohibition, or access condition. Findings must be addressed with explicit rationale before the change is eligible for adoption. A review that produces no findings must document that it was conducted and explain why no issues were identified. _Amendment rationale: PAOS-TEST-0001 through 0007 address new proposed rules; this rule closes the gap for expansions and rewrites, and requires that gaps, edge cases, and loopholes be named and reviewed as distinct categories rather than treated as a single undifferentiated check. Adopted 2026-04-30 per platform maintainer direction; strengthened same day._
